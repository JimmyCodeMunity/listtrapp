        ...
        >
          <div className="py-1">
            {options.map((opt, idx) => (
              <button
                key={idx}
                onClick={() => handleSelect(opt.categoryname)}
                className={clsx(
                  "block w-full px-4 py-2 text-xs text-left text-gray-700 hover:bg-gray-100 hover:text-black",
                  selected === opt.categoryname && "bg-gray-100 font-semibold"
                )}
              >
                {opt.categoryname}
              </button>
